...
void FastExplorationFSM::init(ros::NodeHandle &nh) {
  ...
  fd_->state_str_ = {"INIT",     "WAIT_TRIGGER", "PLAN_TRAJ",
                     "PUB_TRAJ", "EXEC_TRAJ",    "FINISH"};
  fd_->static_state_ = true;
  fd_->trigger_ = false;

  /* Ros sub, pub and timer */
  exec_timer_ = nh.createTimer(ros::Duration(0.05),
                               &FastExplorationFSM::FSMCallback, this);
  safety_timer_ = nh.createTimer(ros::Duration(0.05),
                                 &FastExplorationFSM::safetyCallback, this);
  frontier_timer_ = nh.createTimer(ros::Duration(0.5),
  ...
  planner_state_pub_ = nh.advertise<std_msgs::String>("/planner_state", 10);
}
  ...
  switch (state_) {
    ...
    case PLAN_TRAJ: {
      if (fd_->static_state_) {
        ...
        // Replan from non-static state, starting from 'replan_time' seconds
        // later
        ...
        // if ((fd_->start_pt_ - fd_->odom_pos_).norm() > 2)
        // {
        //   fd_->start_pt_ = fd_->odom_pos_;
        //   fd_->start_vel_ = fd_->odom_vel_;
        //   fd_->start_acc_.setZero();
    ...
    case EXEC_TRAJ: {
      ...
      double time_to_end = info->duration_;
      if ((info->position_traj_.evaluateDeBoorT(time_to_end) - fd_->odom_pos_)
              .norm() < 0.2) {
        transitState(PLAN_TRAJ, "FSM");
        ROS_WARN("Replan位置到达了重规划");
        std_msgs::String msg;
        msg.data = "CONTINUE_PLANNER";  //
        planner_state_pub_.publish(msg);
        return;
      }
...
void FastExplorationFSM::visualize() {
  ...
  // visualization_->drawBox((bmin + bmax) / 2.0, bmax - bmin, Vector4d(0, 1, 0,
  // 0.3), "updated_box", 0, 4);
  ...
  for (int i = 0; i < ed_ptr->frontiers_.size(); ++i) {
    ...
    //                         Vector4d(0.5, 0, 1, 0.3), "frontier_boxes", i,
    //                         4);
  }
  for (int i = ed_ptr->frontiers_.size(); i < last_ftr_num; ++i) {
    visualization_->drawCubes({}, 0.1, Vector4d(0, 0, 0, 1), "frontier", i, 4);
    // visualization_->drawBox(Vector3d(0, 0, 0), Vector3d(0, 0, 0), Vector4d(1,
    // 0, 0, 0.3), "frontier_boxes", i, 4);
  }
  last_ftr_num = ed_ptr->frontiers_.size();
  // for (int i = 0; i < ed_ptr->dead_frontiers_.size(); ++i)
  //   visualization_->drawCubes(ed_ptr->dead_frontiers_[i], 0.1, Vector4d(0, 0,
  //   0, 0.5), "dead_frontier",
  //                             i, 4);
  // for (int i = ed_ptr->dead_frontiers_.size(); i < 5; ++i)
  //   visualization_->drawCubes({}, 0.1, Vector4d(0, 0, 0, 0.5),
  //   "dead_frontier", i, 4);

  // Draw global top viewpoints info
  // visualization_->drawSpheres(ed_ptr->points_, 0.2, Vector4d(0, 0.5, 0, 1),
  // "points", 0, 6); visualization_->drawLines(ed_ptr->global_tour_, 0.07,
  // Vector4d(0, 0.5, 0, 1), "global_tour", 0, 6);
  // visualization_->drawLines(ed_ptr->points_, ed_ptr->views_, 0.05,
  // Vector4d(0, 1, 0.5, 1), "view", 0, 6);
  // visualization_->drawLines(ed_ptr->points_, ed_ptr->averages_, 0.03,
  // Vector4d(1, 0, 0, 1), "point-average", 0, 6);

  // Draw local refined viewpoints info
  // visualization_->drawSpheres(ed_ptr->refined_points_, 0.2, Vector4d(0, 0, 1,
  // 1), "refined_pts", 0, 6);
  // visualization_->drawLines(ed_ptr->refined_points_, ed_ptr->refined_views_,
  // 0.05,
  //                           Vector4d(0.5, 0, 1, 1), "refined_view", 0, 6);
  // visualization_->drawLines(ed_ptr->refined_tour_, 0.07, Vector4d(0, 0, 1,
  // 1), "refined_tour", 0, 6);
  // visualization_->drawLines(ed_ptr->refined_views1_, ed_ptr->refined_views2_,
  // 0.04, Vector4d(0, 0, 0, 1),
  //                           "refined_view", 0, 6);
  // visualization_->drawLines(ed_ptr->refined_points_,
  // ed_ptr->unrefined_points_, 0.05, Vector4d(1, 1, 0, 1),
  //                           "refine_pair", 0, 6);
  ...
  //                               / ed_ptr->frontiers_.size()), "n_points", i,
  //                               6);
  ...
  // Draw trajectory
  // visualization_->drawSpheres({ ed_ptr->next_goal_ }, 0.3, Vector4d(0, 1, 1,
  // 1), "next_goal", 0, 6);
  visualization_->drawBspline(info->position_traj_, 0.1,
                              Vector4d(1.0, 0.0, 0.0, 1), false, 0.15,
                              Vector4d(1, 1, 0, 1));
  // visualization_->drawSpheres(plan_data->kino_path_, 0.1, Vector4d(1, 0, 1,
  // 1), "kino_path", 0, 0); visualization_->drawLines(ed_ptr->path_next_goal_,
  // 0.05, Vector4d(0, 1, 1, 1), "next_goal", 1, 6);
}
...
void FastExplorationFSM::frontierCallback(const ros::TimerEvent &e) {
  ...
  if (state_ == WAIT_TRIGGER || state_ == FINISH) {
    ...
    for (int i = 0; i < ed->frontiers_.size(); ++i) {
      visualization_->drawCubes(
          ...
          "frontier", i, 4);
      // visualization_->drawBox(ed->frontier_boxes_[i].first,
      // ed->frontier_boxes_[i].second, Vector4d(0.5, 0, 1, 0.3),
      //                         "frontier_boxes", i, 4);
    }
    for (int i = ed->frontiers_.size(); i < 50; ++i) {
      visualization_->drawCubes({}, 0.1, Vector4d(0, 0, 0, 1), "frontier", i,
                                4);
      // visualization_->drawBox(Vector3d(0, 0, 0), Vector3d(0, 0, 0),
      // Vector4d(1, 0, 0, 0.3), "frontier_boxes", i, 4);
    }
  }
  ...
  //   if (planner_manager_->path_finder_->search(fd_->odom_pos_, Vector3d(-5,
  //   0, 1)))
  //   {
  //     auto path = planner_manager_->path_finder_->getPath();
  //     visualization_->drawLines(path, 0.05, Vector4d(1, 0, 0, 1), "astar", 0,
  //     6); auto visit = planner_manager_->path_finder_->getVisited();
  //     visualization_->drawCubes(visit, 0.3, Vector4d(0, 0, 1, 0.4),
  //     "astar-visit", 0, 6);
  //   }
  //   astar_num += 1;
