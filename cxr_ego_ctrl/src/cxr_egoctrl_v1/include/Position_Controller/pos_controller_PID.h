...
#include "Position_Controller/math_utils.h"
#include "Position_Controller/controller_utils.h"
#include "Position_Controller/geometry_utils.h"
#include "Position_Controller/printf_utils.h"
...
public:
    ...
    void set_current_odom(const nav_msgs::Odometry &odom)
    {

        nav_msgs::Odometry position_msg = odom;
        current_state.pos(0) = position_msg.pose.pose.position.x;
        current_state.pos(1) = position_msg.pose.pose.position.y;
        current_state.pos(2) = position_msg.pose.pose.position.z;
        current_state.vel(0) = position_msg.twist.twist.linear.x;
        current_state.vel(1) = position_msg.twist.twist.linear.y;
        current_state.vel(2) = position_msg.twist.twist.linear.z;
        current_state.q.w() = position_msg.pose.pose.orientation.w;
        current_state.q.x() = position_msg.pose.pose.orientation.x;
        current_state.q.y() = position_msg.pose.pose.orientation.y;
        current_state.q.z() = position_msg.pose.pose.orientation.z;
        current_state.yaw = geometry_utils::get_yaw_from_quaternion(current_state.q);
    }
    void set_current_state(const mavros_msgs::State &state)
    {
        uav_state = state;
    }
...
void pos_controller_PID::init(ros::NodeHandle &nh)
{
    ...
    // 【参数】悬停油门
    nh.param<float>("pid_gain/hov_percent", ctrl_param.hov_percent, 0.58f);
    // 【参数】XYZ积分上限
