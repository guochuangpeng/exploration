...
#include <attitude_controller/AttitudeControllerConfig.h>
...
class attitudeCtrl 
{
  private:
    ...
    double kw_xy_,kw_xy_ff_,kw_z_,kw_z_ff_;
    ...
    // void flattargetCallback(const quadrotor_msgs::FlatTarget& msg);
    void yawtargetCallback(const std_msgs::Float32MultiArray& msg);
    ...
    enum FlightState {
      WAITING_FOR_HOME_POSE, TAKING_OFF,MISSION_EXECUTION, LANDING, LANDED
    } node_state;
  ...
  public:
    void dynamicReconfigureCallback(attitude_controller::AttitudeControllerConfig &config,uint32_t level);
    attitudeCtrl(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
    ...
    Eigen::Vector3d quaternion_rotate_vector(const Eigen::Vector4d &q,const Eigen::Vector3d &v);
    Eigen::Vector4d quaternion_inv(const Eigen::Vector4d &q);
